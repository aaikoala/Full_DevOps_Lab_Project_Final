...
describe("Budget Categories API", () => {
  it("GET /api/categories returns a list", async () => {
    ...
    expect(res.status).toBe(200); // Check response status
  ...
  it("POST /api/categories creates a category", async () => {
    ...
    expect(res.body).toHaveProperty("id"); // Check response contains an id
    expect(res.body.name).toBe("Transport"); // check the name of the category
  });
  ...
  it("DELETE /api/categories/:id returns 404 if category does not exist", async () => {
    const res = await request(app).delete("/api/categories/999999");  // ry to delete a category that doesn't exist
